<?php
	...
	where chapter_des_2.chapter_code = 'XXII'
	...
	'D0020975' as preferred_label_id ,/*from block_v2*/
	...
	left join block_v2 on block_v2.block_code = category_v2.parent 
	where block_v2.parent = 'XXII'
	order by  BlockCode, preferred_label_id, label_id, BlockStatment
	;";

	$chapter_2 = mysqli_query($objConnect,$Chapter2);
			...
			$i = $chapterResult["SubCategory"];
				...
				if($chapterResult["code_kind"] == "chapter"){
					if($chapterResult["code_kind"] != $d){
						...
						$section->addText("(".$chapterResult["minCat"]."-".$chapterResult["maxCat"].")");
					}
				}
				...
				elseif($chapterResult["code_kind"] == "category"){
					if( $chapterResult["CategoryCode"] != $h){
	...
	$objWriter->save('Chapter&Block&SubCategory_Chapte22.docx');
?>